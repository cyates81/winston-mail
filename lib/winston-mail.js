...
var Mail = exports.Mail = function(options) {
  ...
  this.subject = options.subject || (options.interval ? '{{count}} winston errors' : 'winston: {{level}} {{msg}}')
  ...
  // If mailing on a set interval start Interval timer to send mail
  if (this.interval) {
    this.body = ""
    this.errorCount = 0
    var self = this
    setInterval(function() {
      if (self.body.length > 0) {
        var body = self.body;
        var subject = mustache.render(self.subject, {count: self.errorCount})
        self.body = ""
        self.errorCount = 0
...
Mail.prototype.log = function(level, msg, meta, callback) {
  ...
  if (this.formatter) {
    ...
    var body = (this.timestamp ? new Date().toLocaleString() + ': ' : '') + msg
  ...
  var subject = mustache.render(this.subject, {level: level, msg: msg.split('\n')[0]})
  this.send(subject, body, callback)
}

Mail.prototype.send = function(subject, body, callback) {
  var self = this
  var msgOptions = {
    from: this.from,
    to: this.to,
